...
export async function POST(request: Request) {
  ...
  if (!parsed.success) return NextResponse.json({ error: "구독 정보를 확인해줘." }, { status: 400 });
  ...
  try {
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "구독을 등록하지 못했어." }, { status: 409 });
  }
}
