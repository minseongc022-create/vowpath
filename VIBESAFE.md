...
## 1-4. 연결은 두 번의 선택으로 끝난다

사용자가 하는 일은 **GitHub 연결**과 **저장소 고르기**, 그리고 버튼 한 번이다.

| 예전 | 지금 |
|---|---|
| 저장소 고르기 | 저장소 고르기 |
| **배포 주소 직접 입력** | 자동으로 찾아 채워둠 (맞는지만 확인) |
| 앱 화면에서 [앱 분석하기] 클릭 | 자동 |
| 흐름 목록에서 하나씩 켜기 | `safe`만 자동으로 켬 |
| [지금 확인하기] 클릭 | 자동 |

### 배포 주소를 어떻게 아는가 (`github/detect-url.ts`)

Vercel·Netlify를 GitHub에 연결해두면 배포할 때마다 저장소의 `homepage` 칸과
Deployments를 **그쪽이 채워 넣는다**. 바이브코딩 앱은 거의 다 이 경로로
배포되므로, 주소는 대개 이미 GitHub에 있다. 묻지 않고 찾아서 보여주고
맞는지만 확인받는다.

찾은 주소도 사용자가 직접 입력한 것과 똑같이 SSRF 검사를 통과해야 한다 —
저장소 설정은 누구나 바꿀 수 있고, 우리 워커는 그 주소로 실제 요청을 보낸다.

### 무엇을 자동으로 켜는가 (`onboarding.ts`)

`safe` **하나뿐이다.**

| 등급 | 예 | 자동으로 켜나 |
|---|---|---|
| `safe` | 열기·읽기·검색·로그인 | **켠다** — 눌러도 아무것도 남지 않는다 |
| `caution` | 글 작성·수정 | 안 켠다. 목록으로 보여주고 사용자가 켠다 |
| `blocked` | 결제·발송·삭제 | 절대 안 켠다 |

이 경계는 `flows/safety.ts`의 규칙이 정하고 AI는 관여하지 않는다. 그리고
켠 사실을 숨기지 않는다 — 무엇을 켰는지 화면에 그대로 보여주고,
`flow_auto_enabled` 이벤트로 기록도 남긴다. 자동으로 한 일을 사용자가
나중에 발견하는 것이 가장 나쁘다.

`tests/unit/vibesafe/onboarding.test.mjs`가 이 경계를 고정한다.

---

## 1-5. 색

밝고 깨끗하며 신뢰감 있는 SaaS. 어둡고 기술적인 느낌이 아니다.

색은 전부 `vibesafe/styles/vibesafe.css`의 `:root` 토큰 한 곳에 있다.
화면별로 새 hex를 만들지 않는다 — 새 화면도 `--vs-*`만 쓴다.

| 역할 | 토큰 | 값 |
|---|---|---|
| 배경 | `--vs-bg` | `#f7f9fc` |
| 카드 | `--vs-surface` | `#ffffff` |
| 보조 면 | `--vs-surface-sunken` | `#f3f6fa` |
| 테두리 | `--vs-line` | `#e2e8f0` |
| 제목·본문 | `--vs-ink` | `#0f172a` |
| 보조 설명 | `--vs-muted` | `#64748b` |
| 흐린 것(비활성·미확인) | `--vs-neutral` | `#94a3b8` |
| 브랜드 | `--vs-primary` | `#3478f6` |
| 정상 / 문제 / 확인 필요 | `--vs-ok` / `--vs-danger` / `--vs-warn` | `#22a06b` / `#e5484d` / `#f2b84b` |

### ★ 칠하는 색과 글자색을 따로 두는 이유

상태색은 **면과 점을 칠하기 위한 값**이다. 그대로 글자에 쓰면 읽히지 않는다 —
Warning(`#f2b84b`)을 흰 배경 글자로 쓰면 대비가 **1.79:1**로 사실상 안 보인다
(WCAG AA 기준 4.5:1).

그래서 같은 계열에서 대비를 통과하는 글자 전용 토큰을 따로 둔다.

| | 칠하기 | 글자 | 테두리 |
|---|---|---|---|
| 정상 | `--vs-ok` | `--vs-ok-text` (5.02:1) | `--vs-ok-border` |
| 문제 | `--vs-danger` | `--vs-danger-text` (5.62:1) | `--vs-danger-border` |
| 확인 필요 | `--vs-warn` | `--vs-warn-text` (4.92:1) | `--vs-warn-border` |
| 브랜드 | `--vs-primary` | `--vs-primary-text` (5.17:1) | `--vs-primary-border` |

### 하지 않는 것

- 그라데이션 — 없앴다.
- 짙은 그림자 — 테두리가 구조를 잡고, 그림자는 아주 옅게만.
- 어두운 코드 블록 — 검은 바탕 고정폭 글자는 "여기서부터는 당신 영역이
  아닙니다"라는 신호를 준다. 코드 블록도 나머지 화면과 같은 밝기다.
- 상태색으로 카드 전체 칠하기 — 점·아이콘·배지·얇은 테두리·아주 연한
  배경 틴트까지만.
- 보라색, 검정 CTA.

### 알려진 절충 하나

Primary CTA(`#3478f6`) 위의 흰 글자는 **4.07:1**로, AA 기준(4.5:1)에 살짝
못 미친다. 브랜드 색을 그대로 쓰기로 한 결과다. 엄격히 맞추려면 버튼 배경만
`--vs-primary-hover`(`#2563eb`, 5.17:1)로 바꾸면 된다 — CSS 한 줄이다.
...
```
카드 등록(토스 위젯) → authKey 발급 → billingKey로 교환
                                            │
                        이 계정 첫 구독인가?
                        ├─ 예 → 7일 체험 시작(결제 없음, planKey 즉시 pro)
                        └─ 아니오(재가입 등) → 즉시 첫 결제
                                                              │
                                        cron이 매일 확인 ──── ┘
                                              │
                    체험 종료 1일 전            → 결제 예고 알림 1회
                    체험 종료                   → 첫 결제 시도(실패하면 아래 재시도로)
...
| | 무료 베타 | 프로 |
|---|---|---|
| 가격 | 0원 | 29,900원/월 · 7일 무료체험 (환경변수로 조정) |
| 프로젝트 | 1개 | 5개 |
| 월 검사 | 100회 | 2,000회 |
| 월 앱 분석 | 20회 | 200회 |
| 24/7 감지 + 원인 진단 | 무제한 | 무제한 |
| 자동 수정(PR 생성) | **평생 1회** | 무제한 |

`User.planKey`가 "지금 쓸 수 있는 한도"의 유일한 출처다. 구독 상태가
바뀌는 모든 트랜잭션(체험 시작·첫 결제 성공·갱신 성공·해지 완료·다우닝
강등)이 이 컬럼을 같은 트랜잭션 안에서 함께 바꾼다 — 체험은 시작했는데
한도는 그대로거나, 결제는 됐는데 한도는 그대로인 순간이 생기지 않게 하기
위해서다.

### 원인은 항상 무료, 조치만 유료다 (`billing/repair-credit.ts`)

**감지·원인 진단은 요금제·크레딧과 완전히 무관하게 모든 사고에서 항상
무제한이다.** "왜 고장났는지"를 유료 전환 지렛대로 쓰지 않는다 — 원인을
안 보여주는 건 사용자에게 광고처럼 느껴지고, VibeSafe가 지키려는
정직함과 정반대다.

무료 베타에 걸리는 건 딱 하나, **자동 수정(원인 분석 후 PR 생성)을
평생 몇 번 해주는가**뿐이다: 딱 한 번. `VibesafeUser.freeRepairUsedAt`이
그 한 번을 썼는지 기록하고, `freeRepairIncidentId`가 어느 사고에서
썼는지 기록한다. 크레딧은 **실제로 PR이 열렸을 때만** 소진 처리한다 —
AI가 "안전하게 고칠 방법이 없다"고 판단해 PR을 못 연 시도까지 소진시키면
사용자만 손해이기 때문이다. 같은 사고를 다시 시도하는 것("다른 방법으로
다시 시도")은 막지 않는다 — 첫 시도가 틀렸다고 유일한 무료 체험이
통째로 날아가면 안 된다. 다른(새) 사고에서만 막는다.

이 판단(`canStartFreeRepair`)은 화면(`repair/view.ts`)과 서버 게이트
(`repair/propose-fix.ts`) 양쪽에서 정확히 같은 함수를 쓴다 — 판단이 두
곳에 따로 있으면 나중에 한쪽만 고치는 실수가 난다는 걸 이 프로젝트에서
이미 두 번(orderId, past_due 재시도) 겪었다.

### 7일 무료체험 (`billing/subscription.ts`)

이 계정이 **처음** 구독하는 경우에만 카드 등록이 즉시 결제로 이어지지
않는다 — `hasUsedTrial`이 아직 false면 `startTrial()`이 결제 없이
`status: "trialing"` + `trialEndsAt`(+7일)만 세팅하고, `planKey`는
`activateAfterCharge`와 마찬가지로 즉시 pro로 바꾼다(체험의 의미 자체가
"결제 전에 Pro를 먼저 써보는 것"이라서다). 해지 후 재가입 등으로
`hasUsedTrial`이 이미 true인 계정은 예전처럼 등록 즉시 결제된다 — 체험은
계정당 평생 한 번뿐이다.

cron이 매일:
1. `sendTrialEndingReminders()` — 체험 종료 24시간 전 구간에 든 구독에
   **딱 한 번**(`trialReminderSentAt`) 결제 예고 알림을 보낸다. 이게
   "정직한 체험판"과 "체험판 함정"을 가르는 가장 큰 차이다 — 카드가
   등록돼 있다는 이유로 예고 없이 첫 결제가 나가면 안 된다.
2. `processTrialsEnding()` — `trialEndsAt`이 지난 구독의 첫 결제를
   시도한다. `currentPeriodEnd`가 아직 없어서(결제를 한 번도 안 해봐서)
   그대로 두면 재시도마다 `orderIdFor`의 periodStart가 매번
   달라지므로, 첫 시도에서 `currentPeriodEnd`를 `trialEndsAt`에 고정한
   뒤 기존 `chargeRenewal()`을 그대로 재사용한다(아래 dunning 재시도
   로직을 그대로 물려받는다).

체험 중 해지는 `active` 상태의 해지("이미 낸 달은 끝까지 쓰게 한다")와
다르다 — 아직 낸 돈이 없으므로 그 자리에서 바로 beta로 내리고 예정된
첫 결제 자체를 취소한다.

### 결제 실패해도 바로 안 끊는다 (`billing/subscription.ts`)

카드 한도 초과 같은 건 하루 지나면 풀리는 경우가 많다. 그래서 결제
실패(체험 종료 후 첫 결제 포함)는 `past_due`로 표시하고 cron이 하루
주기로 최대 3번까지 다시 시도한다. 그래도 안 되면 그때 무료 베타로
내리고 사용자에게 알린다.

**해지는 즉시 끊지 않는다(체험 중은 예외).** `active` 상태는 이미 낸
이번 달을 끝까지 쓰게 하고, `cancelAtPeriodEnd`가 켜진 채로 기간이
끝나야 cron이 베타로 전환한다.
...
- `VibesafeSubscription.trialEndsAt` / `hasUsedTrial` — 7일 무료체험의
  종료 시각과 "이 계정이 체험을 이미 써봤는가"(평생 한 번뿐이라 해지 후
  재가입해도 재사용되지 않는다).
...
`repair/risk.ts`, `repair/pipeline.ts`, `trust-ladder.ts`, `billing/repair-credit.ts`
처럼 "규칙을 정하는" 파일은 `"server-only"`도 prisma import도 없는 순수
함수다. `trust.ts`, `repair/view.ts`처럼 "DB에서 모아오는" 파일은 그 순수
함수에 값만 넘긴다. `repair/view.ts`(화면에 무엇을 보여줄지)와
`repair/propose-fix.ts`(실제로 막을지)가 "무료 자동 수정 크레딧을 다시
써도 되는가"를 각자 판단하지 않고 `canStartFreeRepair()` 하나를 그대로
불러 쓰는 게 정확히 이 원칙이다 — 같은 판단이 두 곳에 따로 있으면
한쪽만 고치는 실수가 나고, 이 프로젝트는 그 실수를 이미 두 번(orderId,
past_due 재시도) 했다. 이렇게 나누는 이유는 규칙 하나를 확인하려고 매번 데이터베이스를 준비하지
