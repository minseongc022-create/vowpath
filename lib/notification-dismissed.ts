...
function kvKey(userId: string) {
  return `effiroad:notification-dismissed:${userId}`;
}
...
async function writeFileStore(all: Record<string, string[]>) {
  await mkdir(DATA_DIR, { recursive: true });
  await writeFile(DISMISSED_FILE, JSON.stringify(all, null, 2), "utf-8");
}
...
export async function dismissNotifications(
  ...
  if (ids.length === 0) return getDismissedNotificationIds(userId);

  if (useKvStore()) {
    const existing = (await kv.get<string[]>(kvKey(userId))) ?? [];
    const next = [...new Set([...existing, ...ids])].slice(-1000);
  ...
  const store = await readFileStore();
  const merged = [...new Set([...(store[userId] ?? []), ...ids])].slice(-1000);
  store[userId] = merged;
...
export async function dismissAllNotifications(
  ...
  return dismissNotifications(userId, allIds);
}
