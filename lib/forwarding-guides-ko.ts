...
export function getForwardingGuideSteps(
  ...
  const ringTip =
    ...
    "직원이 받을 수 있도록 15~20초 정도 울리게 한 뒤 Effiroad로 넘기세요.";
  ...
  if (provider === "dialpad") {
    ...
    if (activeScenario === "overflow") {
      ...
      return [
        ...
        `영업 시간에는 샵 번호가 먼저 울리고, 받지 못한 통화만 Effiroad로 갑니다. ${ringTip}`,
  ...
  if (provider === "carrier") {
    ...
    if (activeScenario === "overflow") {
      ...
      return [
        ...
        "메인 샵 번호로 전화해 몇 번 울린 뒤 Effiroad가 받는지 확인하세요.",
  ...
  return [
    ...
    "고객이 실제로 거는 번호로 전화해 Effiroad가 받는지 확인하세요.",
