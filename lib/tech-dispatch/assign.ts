...
export async function handleTechDispatchReply(params: {
  ...
  if (parsed.ref && bookingId && !bookingIdMatchesRef(bookingId, parsed.ref)) {
    return { handled: true, replyBody: "Vowpath: Ref does not match your open offer." };
  }

  if (!bookingId) {
    return { handled: true, replyBody: "Vowpath: No open job offer for you right now." };
  }
  ...
  if (!target || target.status !== "offering" || target.currentTechId !== tech.id) {
    return { handled: true, replyBody: "Vowpath: That offer already closed." };
  }
  ...
  if (accept) {
    ...
    const note = `Assigned to ${tech.name} via Vowpath.`;
  ...
  if (next?.status === "offering") {
    return { handled: true, replyBody: "Vowpath: Passed. Offering to the next tech." };
  }

  return {
    handled: true,
    replyBody: "Vowpath: Passed. Shop will assign manually.",
  };
}
