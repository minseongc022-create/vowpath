...
export function buildDashboardHomeMetrics(
  ...
  const prevOps = buildOperationsMetrics(
    calls,
    jobs,
    prevRange.start,
    prevRange.end,
  ...
  const trend30 = buildMissedCallsAnalytics(calls, shop, trend30Start, now, {
    jobs,
    jobberBookings,
    requestStatuses,
  });

  const rangeLabel = monthRange.start.toLocaleDateString("ko-KR", {
    month: "short",
    day: "numeric",
  });
  const rangeEndLabel = monthRange.end.toLocaleDateString("ko-KR", {
    month: "short",
    day: "numeric",
  });
    ...
    insights: [
      {
        ...
        deltaPct: pctChange(
          monthAnalytics.afterHoursCalls,
          prevAnalytics.afterHoursCalls,
        ),
        tone: "brand",
      },
      {
