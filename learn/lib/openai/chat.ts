import { getLearnOpenAiKey, getLearnOpenAiModelEconomy } from "@/learn/lib/openai-config";

const DEFAULT_TIMEOUT_MS = 20_000;

export type LearnChatTurn = { role: "user" | "assistant" | "system"; content: string };

export async function learnOpenAiTextCompletion(params: {
  messages: LearnChatTurn[];
  temperature?: number;
  timeoutMs?: number;
  model?: string;
}): Promise<string> {
  ...
  try {
    res = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        model: params.model ?? getLearnOpenAiModelEconomy(),
