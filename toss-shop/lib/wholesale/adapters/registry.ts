...
const pendingAdapters: WholesaleAdapter[] = [
  pendingAdapter({
    ...
    specNote:
      "위탁 전문·상품수 약 500만개로 도매매와 비슷한 규모. " +
      "⚠️ 다만 공개 개발자 API는 아직 없다 — 자사 API 센터가 '타사 계정 API 연동 준비 중'으로 " +
      "안내하고 있고, 마켓 연동은 자체 통합솔루션(다팔자)을 통해서만 제공된다. " +
      "연동하려면 고객센터를 통한 제휴 문의가 선행돼야 한다.",
  }),
  pendingAdapter({
    platform: "onch",
    label: "온채널",
    ...
    specNote:
      "위탁 전문, 제조사 직배송 비중 높음. 규모는 도매꾹·오너클랜보다 작으나 " +
      "상품정보(스마트스토어 상위노출에 쓰이는 항목)를 명확히 제공하는 것이 강점. " +
      "스마트스토어·쿠팡·11번가·G마켓·옥션과 API 연동을 지원한다고 안내하나, " +
      "그건 마켓 연동이지 서드파티 상품조회 API가 공개됐다는 뜻은 아니다 — 제휴 문의 필요.",
  }),
  pendingAdapter({
    platform: "zentrade",
    label: "젠트레이드",
