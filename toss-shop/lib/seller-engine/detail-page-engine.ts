...
function buildSellingPoints(
  ...
  // ⚠️ 이 목록은 **셀러 전략 용어**다 — 고객에게 보여선 안 되는 것이 섞여 있다.
  //
  // 실측: "카탈로그 차별(구성·용량)"이 상세페이지 셀링포인트로 그대로 나갔다.
  // 아래 정규식이 /구성|용량/에 걸려서다. 그건 "이 상품의 구성이 이렇다"가
  // 아니라 "카탈로그 경쟁을 피하려고 구성을 다르게 짠다"는 우리 쪽 전술
  // 이름이다. 고객이 읽으면 무슨 말인지 모르고, 무엇보다 상품 설명이 아니다.
  //
  // 그래서 이제 **차단 목록을 먼저 통과**해야 한다. 전술 제목에 우리 내부
  // 개념(카탈로그·대표아이템·마진·전술·소싱·경쟁)이 들어 있으면 고객용
  // 문구가 될 수 없다 — 그건 셀러가 읽는 말이다.
  const SELLER_JARGON =
    /카탈로그|대표\s*아이템|item\s*winner|마진|수익|전술|소싱|공급처|경쟁사|노출|랭킹|키워드|광고|수수료|정책|게이트/i;
  const specHighlights = (pick.topSellerPlaybook?.tactics ?? [])
    .filter(
      (t) =>
        t.applied &&
        /구성|규격|용량|사이즈|색상|세트/.test(t.title) &&
        !SELLER_JARGON.test(t.title),
    )
    .slice(0, 2)
    .map((t) => t.title);
