 ...
 *
 * 90% 신뢰도는 "가짜 숫자"가 아니라 **게이트 통과 + 실데이터 가중**으로만 부여됩니다.
 * 데모/미연동 시 90% 미만으로 캡 · 연동 완료 + 모든 게이트 통과 시에만 90+.
 */
...
export const JARVIS_VERSION = "1.0";
export const JARVIS_CONFIDENCE_THRESHOLD = 90;
export const JARVIS_MONTHLY_GOAL_KRW = 10_000_000;

/** 대박(잭팟) 소싱 최소 신뢰도 */
export const JARVIS_JACKPOT_THRESHOLD = 90;
...
export function computeJarvisConfidence(input: {
  ...
  gates.push({
    id: "margin",
    label: "순마진",
    passed: input.marginPct >= 14,
    weight: 14,
    detail: `마진 ${input.marginPct}% ${input.marginPct >= 14 ? "✓" : "(14% 미만)"}`,
  });

  gates.push({
    id: "master",
    label: "Jarvis 종합점수",
    passed: input.v6MasterScore >= 78,
    weight: 16,
  ...
  for (const g of gates) {
    ...
    else confidencePct += g.weight * 0.35;
  }

  if (!input.integration.readyFor90) {
    confidencePct = Math.min(confidencePct, 89);
  }

  if (input.criticalRisks > 0 || input.blockRisks > 0) {
    confidencePct = Math.min(confidencePct, 75);
  }

  if (!input.wholesaleLive && input.integration.wholesaleApi) {
    confidencePct = Math.min(confidencePct, 85);
  }

  confidencePct = Math.min(99, Math.max(0, Math.round(confidencePct)));

  const allHardGates =
    gates.filter((g) => ["integration", "margin", "safety", "domeme_moq"].includes(g.id)).every((g) => g.passed) &&
  ...
  if (input.monthlyProfitKrw >= 900_000 && input.marginPct >= 18 && catalogOk) {
    jackpotPct = Math.min(99, confidencePct + 5);
  }
  if (input.competitionIntensity < 1.2 && input.searchVolume >= 8000) {
    jackpotPct = Math.min(99, jackpotPct + 3);
  }
  ...
  const brief = certified
    ? `Jarvis 인증 ${confidencePct}% — 월 1,000만 원 경로에 포함 가능한 **고신뢰 SKU**`
    : jackpotPct >= 85
      ? `Jarvis ${confidencePct}% — ${failed.length ? failed.join(", ") + " 보완 시 90%+" : "게이트 보완 중"}`
      : `Jarvis ${confidencePct}% — 연동·마진·정책 게이트 통과 필요 (현재 ${failed.join(", ") || "검토 중"})`;

  const monthlyPathNote = certified
    ? `대박 확률 ${jackpotPct}% · 이 SKU만으로 목표 ${Math.round((input.monthlyProfitKrw / JARVIS_MONTHLY_GOAL_KRW) * 1000) / 10}%`
    : `90% 인증까지: ${failed.slice(0, 2).join(" → ")}`;
...
export function buildJarvisMonthlyBrief(input: {
  ...
  if (!input.integration.readyFor90) {
    return `Jarvis: 연동 ${input.integration.score}% — ${input.integration.missing.join(", ")} 완료 후 90%+ 인증 SKU만 월 1,000만 원 경로에 사용합니다.`;
  }
  if (certified.length === 0) {
    return `Jarvis: 연동 완료 — 현재 90% 인증 SKU 0개. 게이트 통과 SKU만 노출하도록 재분석 중입니다.`;
  }
  const pct = Math.min(100, Math.round((projected / goal) * 1000) / 10);
  return `Jarvis: **90% 인증 SKU ${certified.length}개** · 인증 SKU만 합산 월 ${projected.toLocaleString()}원 (${pct}%). 인증 SKU에 80% 시간 투입 시 월 1,000만 원 **실행 경로 ${pct >= 100 ? "달성" : pct >= 70 ? "유효" : "구축 중"}**.`;
}
