...
import { computeAdBreakeven } from "./ad-economics";

export const GROWTH_LEVERS_VERSION = "1.1";
 ...
 * 광고 손익분기 CPC.
 *
 * 손익분기 계산 자체는 단일 진실원(ad-economics.ts의 computeAdBreakeven)에
 * 맡긴다 — 예전엔 여기서 "판매가 × 8% × 전환율"만 계산해 **단위 마진을
 * 완전히 빼먹었다**(입력으로 grossMarginKrw를 받아놓고 쓰지 않았다). 그
 * 결과 2만원 상품의 손익분기가 32원까지 떨어져, winner-sku-engine이 확인한
 * 진짜 효자 SKU에도 "광고 못 건다"는 판단이 나갔다. 이 함수는 이제 그
 * 결과를 바탕으로 run/reduce_bid/stop 같은 배분 판단만 얹는다.
 */
export function computeAdEconomics(input: AdEconomicsInput): AdEconomics {
  ...
  const breakevenResult = computeAdBreakeven({
    ...
    netProfitPerUnitKrw: input.grossMarginKrw,
    conversionRatePct: input.conversionRatePct,
    alreadyFeeFree: input.alreadyFeeFree,
  });
  const breakeven = breakevenResult.breakevenCpcKrw;
  const feeSaved = breakevenResult.feeSavedPerSaleKrw;

  if (input.grossMarginKrw <= 0) {
    return {
      breakevenCpcKrw: 0,
      feeSavedPerSaleKrw: feeSaved,
      recommendation: "cannot_bid",
      reason: "이 가격에서는 순이익이 없다 — 어떤 CPC도 광고를 정당화하지 못한다",
    };
  }

  const costPerSale = input.currentCpcKrw ? Math.round(input.currentCpcKrw / cvr) : undefined;
  const netDelta = costPerSale !== undefined ? breakevenResult.valuePerSaleKrw - costPerSale : undefined;

  let recommendation: AdEconomics["recommendation"] = "run";
  let reason =
    `손익분기 CPC ${breakeven.toLocaleString()}원 — 이하로 입찰하면 판매 1건 가치(${breakevenResult.valuePerSaleKrw.toLocaleString()}원)가 ` +
    "광고비보다 커서 순이익이 는다";
  ...
  if (input.currentCpcKrw !== undefined) {
    if (input.currentCpcKrw > breakeven) {
      ...
      reason =
        `현재 CPC ${input.currentCpcKrw}원 > 손익분기 ${breakeven.toLocaleString()}원 — 건당 ${Math.abs(netDelta ?? 0).toLocaleString()}원 손해. ` +
        `${breakeven.toLocaleString()}원 이하로 낮출 것`;
    } else {
      reason = `현재 CPC ${input.currentCpcKrw}원 ≤ 손익분기 ${breakeven.toLocaleString()}원 — 건당 ${(netDelta ?? 0).toLocaleString()}원 이득`;
    }
  }
