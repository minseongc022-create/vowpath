...
function buildDomemeOrderPayload(order: TossOrderProduct, draft?: JarvisListingDraft): {
  receiverName: string;
  ...
  return {
    ...
    itemNo,
    note: itemNo
      ? `도매매 ${itemNo} — ${order.receiverName} / ${order.address} ${order.detailAddress} (${order.zipCode}) · ${order.quantity}개`
...
export async function processFulfillmentCycle(input: {
  ...
  for (const order of orders) {
    ...
    const domemePayload = buildDomemeOrderPayload(order, draft);
    ...
    const job: JarvisFulfillmentJob = {
      ...
      customer: {
        ...
        address: domemePayload.address,
        zipCode: order.zipCode,
      },
      quantity: order.quantity,
      wholesalePlatform: draft?.listingPayload.supplierPlatform,
      supplierUrl: domemePayload.supplierUrl,
      itemNo: domemePayload.itemNo,
      domemeOrderNote: domemePayload.note,
