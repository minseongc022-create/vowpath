...
import { needsSupplierAddress, readSupplierReturnPolicy } from "../wholesale/supplier-return-policy";
    ...
    for (const pick of certified as ConsignmentPick[]) {
      ...
      try {
        ...
        if (pick.wholesaleBest) {
          ...
          // 비용(왕복비)이 든다는 걸 프로비저닝 큐에 남겨, 반복해서 걸리고
          // 돈이 되는 공급처는 사장님께 등록을 권한다. 등록되면 다음 사이클부터
          // 그 비용이 사라진다.
        ...
        // 공급처 전용 주소가 아직 없어 셀러 주소로 강제 폴백된 경우 — 등록은
        // 정상 진행하되(사람 개입 없음), 반품이 실제로 오면 사장님이 받아서
        // 처리해야 한다는 걸 체크리스트에 남긴다. 막지는 않는다.
        const forcedSellerFallback =
          pick.wholesaleBest != null &&
          sellerOwnedLocationId != null &&
        ...
        if (forcedSellerFallback) {
          draft.sellerChecklist = [
            "ℹ 이 상품은 공급처 전용 반품지가 아직 없어 셀러 주소로 등록됐습니다 — 반품이 오면 공급처로 재발송이 필요할 수 있습니다 (비용은 마진에 이미 반영)",
            ...draft.sellerChecklist,
          ];
        }
