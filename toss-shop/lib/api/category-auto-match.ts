...
function pickBranchByName(input: {
  ...
  for (const w of [...words]) {
    // 정확히 일치하는 낱말만 찾으면 안 된다 — 상품명은 낱말이 붙어 온다.
    // "볶음참깨"는 "참깨"가 아니고, "컵라면"은 "라면"이 아니며, "냉동만두"는
    // "만두"가 아니다. 사전 열쇠가 낱말 안에 들어 있으면 같이 본다.
    for (const [key, syns] of Object.entries(TERM_EXPANSIONS)) {
      if (w === key || w.includes(key)) {
        for (const syn of syns) words.add(syn);
      }
    }
  }
...
const TERM_EXPANSIONS: Record<string, string[]> = {
  ...
  주걱: ["주방", "조리"], 채반: ["주방", "조리"], 앞치마: ["주방"],
  // ── 청소·생활 ──
  걸레: ["청소"], 밀대: ["청소"], 수세미: ["청소", "주방"], 쓰레기통: ["청소", "생활"],
  옷걸이: ["생활", "수납"], 정리함: ["수납", "생활"], 선반: ["수납", "가구"],
  빨래건조대: ["생활", "세탁"], 제습제: ["생활"],
  // ── 욕실 ──
  칫솔: ["욕실", "구강"], 샤워기: ["욕실"], 발매트: ["욕실", "매트"], 수건: ["욕실", "타월"],
  // ── 뷰티 ──
  크림: ["스킨케어", "기초"], 세럼: ["스킨케어", "기초"], 토너: ["스킨케어", "기초"],
  선크림: ["선케어", "스킨케어"], 마스크팩: ["마스크", "스킨케어"], 클렌징: ["클렌징", "스킨케어"],
  립밤: ["립", "메이크업"], 쿠션: ["베이스", "메이크업"], 샴푸: ["헤어", "바디"],
  // ── 디지털 ──
  케이블: ["케이블", "액세서리"], 충전기: ["충전", "액세서리"],
  이어폰: ["이어폰", "음향"], 마우스: ["마우스", "주변기기"], 키보드: ["키보드", "주변기기"],
  거치대: ["거치대", "액세서리"], 보조배터리: ["충전", "액세서리"],
  // ── 패션 ──
  ...
  //   장/소스 · 전통주 · 커피/차
  // 이 이름들에 실제로 걸리는 낱말만 넣는다.
  라면: ["가공", "즉석식품"], 즉석밥: ["가공", "즉석식품"], 통조림: ["가공"],
  참깨: ["가루", "조미료", "향신료"], 소금: ["가루", "조미료"], 설탕: ["가루", "조미료"],
  후추: ["가루", "향신료"], 밀가루: ["가루"], 미숫가루: ["가루"],
  홍삼: ["건강식품"], 비타민: ["건강식품"], 유산균: ["건강식품"], 영양제: ["건강식품"],
  만두: ["냉장", "냉동식품"], 돈까스: ["냉장", "냉동식품"], 어묵: ["냉장", "냉동식품"],
  생수: ["생수", "음료"], 음료: ["음료"], 주스: ["음료"], 탄산수: ["생수", "음료"],
  과자: ["스낵", "간식"], 젤리: ["스낵", "간식"], 초콜릿: ["스낵", "간식"],
  견과: ["스낵", "간식"], 육포: ["스낵", "간식"],
  쌀: ["신선식품"], 과일: ["신선식품"], 채소: ["신선식품"], 계란: ["신선식품"],
  우유: ["유제품"], 요거트: ["유제품"], 치즈: ["유제품"], 아이스크림: ["아이스크림"],
  간장: ["장", "소스"], 고추장: ["장", "소스"], 된장: ["장", "소스"], 소스: ["소스"],
  식초: ["장", "소스"], 참기름: ["장", "소스"],
  막걸리: ["전통주"], 소주: ["전통주"],
  커피: ["커피"], 원두: ["커피"], 녹차: ["차"], 티백: ["차"],
};
