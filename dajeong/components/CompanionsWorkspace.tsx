...
export function CompanionsWorkspace() {
  ...
  async function createInvite() {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.error || "초대를 만들지 못했어요.");
  ...
  async function acceptInvite(event: FormEvent) {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || "연결하지 못했어요.");
      setAcceptCode("");
      setStatus("동반자로 연결됐어요.");
      await refresh(myId);
    } catch (error) {
      setStatus(error instanceof Error ? error.message : "잠시 후 다시 시도해 주세요.");
    } finally {
  ...
  async function removeCompanion(linkId: string) {
    if (!window.confirm("이 동반자와 연결을 해제할까요? 공유했던 계획도 함께 볼 수 없게 돼요.")) return;
  ...
  return (
    <div className="dj-plans-page dj-narrow dj-companions-page">
      <div className="dj-plans-heading">
        <div><span className="dj-kicker"><HeartIcon size={15} /> 함께 준비하는 사람들</span><h1>동반자</h1><p>연결한 동반자와 계획을 공유하고, 서로 허용한 부분만 함께 볼 수 있어요.</p></div>
      ...
      <section className="dj-card dj-companion-invite">
        ...
        {invite ? (
          <div className="dj-invite-code-box">
            ...
            <p>이 코드를 동반자에게 알려주면, 아래 “초대 코드로 연결하기”에 입력해서 연결할 수 있어요. 7일 동안 유효해요.</p>
          </div>
        ) : null}
      </section>
      ...
      <section className="dj-card dj-companion-accept">
        ...
        <form onSubmit={acceptInvite}>
          <input value={acceptCode} onChange={(event) => setAcceptCode(event.target.value.toUpperCase())} placeholder="받은 코드를 입력하세요" maxLength={12} />
      ...
      <section>
        ...
        {companions.length === 0 ? <p className="dj-companion-empty">아직 연결된 동반자가 없어요.</p> : (
      ...
      <section>
        ...
        {sharedWithMe.length === 0 ? <p className="dj-companion-empty">아직 공유받은 계획이 없어요.</p> : (
          <div className="dj-plans-list">
            {sharedWithMe.filter((entry) => entry.plan).map((entry) => (
              <Link key={entry.planId} href={`/dajeong/plan/${entry.planId}/today`} className="dj-saved-plan dj-card dj-saved-plan-main">
                ...
                <div><div className="dj-saved-meta"><span><ClockIcon size={13} />{entry.plan?.situation.targetDate ? displayDate(entry.plan.situation.targetDate) : ""}</span></div><h2>{entry.plan?.title}</h2><p>{entry.ownerName}님이 공유했어요</p></div>
                <ArrowIcon size={18} />
              </Link>
            ))}
          </div>
