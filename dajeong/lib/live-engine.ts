...
function reservationReminder(plan: DajeongPlan, item: PlanItem): string | null {
  ...
  return `${item.title}은 예약 확인이 아직 끝나지 않았어요 (${item.reality?.reservationLabel ?? "확인 필요"}).`;
}
...
export function buildLiveSnapshot(plan: DajeongPlan, nowClock: string = currentClock(), dayNumber: number = resolveDayNumber(plan)): LiveSnapshot {
  ...
  const weatherNote = weatherDay && weatherDay.impact !== "low"
    ? `${weatherDay.impact === "high" ? "비·눈 영향이 커요" : "날씨 변화가 있을 수 있어요"} · ${marked.schedule?.weather.message ?? ""}`
  ...
  const prepReminders = [
    ...
    next?.travelFromPrevious?.weatherExposure === "high" ? `${next.title}로 이동은 비·눈에 노출될 수 있어요. 우산이나 여유 시간을 챙기세요.` : null,
    current?.reality?.openNow === false ? `${current.title}은 지금 영업 종료로 표시돼요. 현장에서 다시 확인해 주세요.` : null,
...
export function applyDelayReport(plan: DajeongPlan, input: DelayReportInput): LiveActionResult {
  ...
  if (!target) return { plan, message: "지금 어떤 일정 이야기인지 정확히 못 짚었어요. 장소 이름을 함께 말해 주세요.", changedItemIds: [] };
  ...
  const message = [
    `${target.title}이(가) 예정보다 늦어지는 걸 반영해서 지금 머무는 시간을 약 ${newDuration}분으로 다시 잡았어요.`,
    shiftedTitles.length ? `이어서 ${shiftedTitles.join(", ")} 시작 시간을 뒤로 밀었어요.` : null,
    removedTitles.length ? `그래도 시간이 부족해서 이번엔 ${removedTitles.join(", ")} 일정은 뺐어요. 원하면 다시 넣거나 다른 날로 옮길 수 있어요.` : null,
    !shiftedTitles.length && !removedTitles.length ? "다른 일정은 그대로 유지했어요." : null,
...
export function applyStayLonger(plan: DajeongPlan, input: StayLongerInput): LiveActionResult {
  ...
  const dayNumber = resolveDayNumber(plan);
  const target = resolveTarget(plan, now, dayNumber, input.itemId);
  if (!target) return { plan, message: "어떤 일정에 더 있고 싶은지 알려주세요.", changedItemIds: [] };
  ...
  const { removedTitles, shiftedTitles } = diffAfterReschedule(plan, next, target.id);
  const message = [
    `${target.title}에서 ${extra}분 더 머무는 걸로 바꿨어요.`,
    shiftedTitles.length ? `${shiftedTitles.join(", ")}은 뒤로 조금씩 밀렸어요.` : "다른 일정 시간은 그대로예요.",
    removedTitles.length ? `${removedTitles.join(", ")}은 시간이 부족해 이번엔 뺐어요.` : null,
  ].filter(Boolean).join(" ");
...
export function applyLeaveEarly(plan: DajeongPlan, input: LeaveEarlyInput): LiveActionResult {
  ...
  const message = [
    `오늘은 ${homeByTime}쯤 마무리하는 걸로 맞췄어요.`,
    removedTitles.length ? `남은 일정 중 ${removedTitles.join(", ")}은 이번엔 빼기로 했어요.` : null,
    shiftedTitles.length ? `${shiftedTitles.join(", ")}은 앞당겨서 진행해요.` : null,
    next.schedule?.estimatedHomeArrival ? `예상 귀가는 ${next.schedule.estimatedHomeArrival}쯤이에요.` : null,
  ].filter(Boolean).join(" ");
...
export function applySkipNext(plan: DajeongPlan, input: SkipNextInput): LiveActionResult {
  ...
  if (!target) return { plan: marked, message: "뺄 다음 일정을 찾지 못했어요. 장소 이름으로 말해 주세요.", changedItemIds: [] };
  if (target.placeLocked) return { plan: marked, message: `${target.title}은 꼭 유지하기로 고정한 일정이라 그대로 두었어요. 먼저 고정을 해제하면 뺄 수 있어요.`, changedItemIds: [] };
  const next = scheduleDajeongPlan({ ...marked, items: marked.items.filter((item) => item.id !== target.id) });
  return { plan: next, message: `${target.title} 일정은 빼고 남은 시간표를 다시 맞췄어요.`, changedItemIds: [target.id] };
}
