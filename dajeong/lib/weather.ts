...
export async function fetchWeatherContext(plan: DajeongPlan): Promise<WeatherContext> {
  ...
  if (!coordinates) return unavailable("unavailable", `${plan.situation.region}의 예보 위치를 정확히 연결하지 못했어요.`);
  ...
  if (distance < 0 || distance > 15) return unavailable("outside_forecast", "현재 신뢰할 수 있는 시간대별 예보 범위 밖이라 날씨를 확정하지 않았어요.");
  ...
  try {
    ...
    if (!response.ok) return unavailable("unavailable", "날씨 서비스가 응답하지 않아 예보를 일정에 확정 반영하지 않았어요.");
    const days = normalize(await response.json() as OpenMeteoResponse, start, finish);
    if (!days.length) return unavailable("unavailable", "시간대별 예보를 받지 못해 날씨를 확정하지 않았어요.");
    return {
      status: "verified", sourceLabel: "Open-Meteo", sourceUrl: "https://open-meteo.com/", checkedAt: new Date().toISOString(), days,
      message: "시간대별 강수·기온·체감온도·바람을 확인해 장소와 이동 노출을 함께 검토했어요.",
    };
  } catch {
    return unavailable("unavailable", "날씨 데이터를 확인하지 못해 기존 일정에 임의의 예보를 적용하지 않았어요.");
  }
}
