  ...
  if (step.kind === "menu") {
    return (
      <div className="mt-3 space-y-2">
        <p className="text-[10px] font-bold uppercase tracking-wider text-sky-300/80">{step.prompt}</p>
        {step.options.map((opt) => (
          <button
            key={opt.id}
            type="button"
            onClick={() => onMenuChoice(opt)}
            className="block w-full rounded-xl bg-sky-500/25 px-3 py-2.5 text-left text-xs font-semibold text-white ring-1 ring-sky-400/40 transition hover:bg-sky-500/40 sm:text-sm"
          >
            {opt.label}
          </button>
        ))}
  ...
  if (step.kind === "customer-action") {
    return (
      <button
        ...
        className="mt-3 block w-full rounded-xl bg-sky-500/30 px-3 py-2.5 text-left text-xs font-semibold text-white ring-2 ring-sky-400/50 transition hover:bg-sky-500/45 sm:text-sm"
      >
        Tap: {step.label}
      </button>
    );
  ...
  if (step.kind === "owner-action") {
    return (
      <button
        ...
        className="mt-3 block w-full rounded-xl bg-emerald-500/25 px-3 py-2.5 text-left text-xs font-semibold text-emerald-50 ring-2 ring-emerald-400/50 transition hover:bg-emerald-500/40 sm:text-sm"
      >
