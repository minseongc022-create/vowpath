...
import { demoSummaryHvac } from "@/lib/content-marketing-hvac-en";
import type { DemoVertical } from "@/lib/demo-vertical-config";

type DemoSummaryProps = {
  vertical?: DemoVertical;
};

export function DemoSummary({ vertical = "restoration" }: DemoSummaryProps) {
  const { demoSummary } = useSiteContent();
  const content = vertical === "hvac" ? demoSummaryHvac : demoSummary;

  return (
    <section className="border-b border-brand-200/40 bg-brand-50 py-14 sm:py-16">
      <Container>
        <div className="mx-auto max-w-3xl text-center">
          <h2 className="text-xl font-bold tracking-tight text-brand-900 sm:text-2xl">{content.title}</h2>
          <p className="mt-2 text-sm text-stone-600 sm:text-base">{content.subtitle}</p>
        </div>
