...
export function Hero() {
  ...
  return (
    <section className="vow-hero relative overflow-hidden text-white">
      ...
      <div
        className="pointer-events-none absolute left-1/2 top-0 h-[36rem] w-[min(100%,52rem)] -translate-x-1/2 rounded-full bg-teal-500/20 blur-3xl"
        aria-hidden
      />

      <Container className="relative py-10 sm:py-14 lg:py-20">
        <div className="mx-auto max-w-4xl text-center">
          <p className="mb-4 inline-flex rounded-full border border-teal-300/30 bg-teal-500/15 px-4 py-1 text-xs font-semibold uppercase tracking-wider text-teal-100">
            {h.badge}
          </p>
          <h1 className="text-4xl font-bold tracking-tight sm:text-5xl lg:text-6xl">
            {h.headline}
            <span className="mt-1 block bg-gradient-to-r from-teal-200 via-brand-300 to-amber-300 bg-clip-text text-transparent">
              {h.headlineAccent}
            </span>
          </h1>
          {"brandLine" in h && h.brandLine ? (
            <p className="mx-auto mt-5 max-w-2xl text-base font-medium text-teal-100/90 sm:text-lg">
              {h.brandLine}
            </p>
          ) : null}
          ...
          <ul className="mx-auto mt-8 flex max-w-4xl flex-wrap items-center justify-center gap-x-6 gap-y-3">
            {badges.map((badge) => (
              <li key={badge} className="flex items-center gap-2 text-sm text-slate-300">
                <span
                  className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-teal-500/20 text-teal-200"
                  aria-hidden
                >
