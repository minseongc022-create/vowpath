    ...
    <section className="vow-dash-panel">
      ...
      <div className="grid gap-4 p-5 sm:grid-cols-2 lg:grid-cols-4">
        {busy ? (
          ...
          <>
            ...
            />
            <MetricCard
              label="Vowpath-attributed"
              value={m.attributedCollectedLabel}
              hint={`${m.attributedInvoiceCount} invoice${m.attributedInvoiceCount === 1 ? "" : "s"} linked to Vowpath intake`}
      ...
      {m && !busy && !error ? (
        <p className="border-t border-brand-200/50 px-5 py-3 text-xs text-stone-500">
          ...
          Vowpath-attributed links invoices to AI calls or bookings via Jobber request / job IDs.
