...
import { shopVerticalLabel, type ShopVertical } from "@/lib/shop-vertical";
      ...
      <aside className="vow-dash-sidebar">
        <div className="flex h-full w-full flex-col px-4 py-5">
                ...
                >
                  <span
                    className={`vow-dash-nav-icon ${
                      active ? "bg-brand-500/20 text-brand-200" : "bg-white/[0.04] text-slate-500"
                    }`}
                  >
                    {item.icon}
                  </span>
          ...
          <div className="vow-dash-card mt-auto !rounded-2xl !border-brand-500/25 !bg-[#3d3228]">
            ...
            <p className="mt-1.5 text-sm leading-relaxed text-slate-300">{vowDashboard.upgrade.body}</p>
            <Link href={ROUTES.pricing} className="vow-dash-btn-primary mt-3 w-full text-center text-sm">
              {planCta}
            </Link>
          </div>
