...
| `redirect_uri` 오류 (isn't valid / 유효하지 않습니다) | Jobber Developer Center에서 **Vercel과 같은 Client ID** 앱을 연 뒤 OAuth Callback URL에 `https://effiroad.com/api/jobber/callback` 을 **공백 없이** 붙여넣고 **Save**. (칸이 하나뿐이면 localhost를 이 URL로 교체.) 저장 후 Settings에서 Connect → Allow Access. |
