...
// ─────────────────────────────────────────────────────────────
// 실제 사고: 검색어를 24개만 훑어서 "왜 이렇게 좁게 찾나" 불만이 나왔다.
// 검색어 풀 자체가 넉넉해야 넓게 찾을 여지가 생긴다.
// ─────────────────────────────────────────────────────────────

test("검색어 풀이 한 사이클 기본 훑기 범위보다 훨씬 크다 — 회전할 여지가 있다", () => {
  assert.ok(keywordPoolSize() > 150, `검색어가 ${keywordPoolSize()}개뿐이면 금방 바닥난다`);
});

test("기본 훑기 범위 자체도 넓게 잡혀 있다", () => {
  assert.ok(getKeywords().length >= 60, "기본값이 좁으면 매 사이클 좁게 찾는다");
});
