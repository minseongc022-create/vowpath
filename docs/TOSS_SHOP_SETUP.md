...
토스는 반품지를 **새로 만드는 API를 열어두지 않습니다**(공식 FAQ 기준). 그래서 새 주소를 토스에 등록하는 것만은 셀러센터에서 사람이 해야 합니다.

다만 자비스는 막힌 공급처를 전부 떠넘기지 않습니다. **반복해서 걸리고 실제로 돈이 되는 곳만 최대 3곳** 추려 대시보드에 띄웁니다. 나머지는 등록 없이 팔 수 있는 후보로 채웁니다.

등록할 때는 자비스가 알려주는 이름을 **그대로** 쓰세요.

```
이름: 자비스-domeme-abc123      ← 이 이름이 있으면 주소 판독이 실패해도 자동 연결됩니다
주소: 경기 화성시 동탄대로 45
```

한 번 등록하면 그 공급처 상품은 **이후 전부 자동**입니다. 손대는 횟수는 시간이 갈수록 줄어듭니다.
