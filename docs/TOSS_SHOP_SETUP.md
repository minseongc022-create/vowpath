...
#### 완전 자동화 — 반품지 없다고 소싱을 멈추지 않습니다

등록 API가 없는 이상, "공급처 주소가 등록될 때까지 기다린다"는 곧 "사람이 계속 손봐야 자동화가 된다"는 뜻입니다. 사장님 지시로 그 제약을 없앴습니다: **공급처 전용 반품지가 없어도 셀러 반품지로 강제 폴백해서 무조건 등록합니다.**

| 공급처 유형 | 자비스 동작 |
| --- | --- |
| 반품 안내 없음 (절대다수) | 셀러 반품지로 자동 등록 + 왕복비를 마진에서 미리 차감 |
| 셀러 처리형으로 확인 | 셀러 반품지로 자동 등록 |
| 공급처 수거형·택배 반송형, 주소가 토스에 있음 | 그 주소로 자동 연결 (비용 0원, 최선) |
| 공급처 수거형·택배 반송형, 주소가 토스에 없음 | **셀러 반품지로 강제 등록** — 왕복비(재발송 포함)를 충당금으로 미리 차감. 등록되면 다음 사이클부터 자동으로 위 줄(비용 0원)로 승격 |
| 반품 불가 | 소싱 제외 — 이것만은 등록해도 실제로 못 파는 경우라 예외입니다 |

**유일하게 예외인 경우:** 반품 자체가 안 되는 공급처(토스가 청약철회를 보장해야 하므로 이건 팔면 무조건 손해), 그리고 충당금을 빼면 마진이 안 남는 경제성 미달 SKU. 이 둘은 "반품지 문제"가 아니라 "이 상품 자체가 안 되는" 경우라 자동화로 풀 수 없습니다.

도매매 공급사는 수천 곳이고, 지금은 그 어느 것도 반품지 미등록만으로는 막히지 않습니다.

#### 그래도 값어치가 있는 곳은 등록을 권합니다 — 비용을 0으로 만드는 방법

강제 폴백은 "됐다"이지 "공짜"는 아닙니다. 반품이 실제로 오면 사장님이 받아서 공급처로 재발송해야 할 수 있고, 그 왕복비가 마진에서 계속 빠져나갑니다. 그래서 반복해서 걸리고 실제로 돈이 되는 공급처는 **최대 3곳**만 대시보드에 띄워, 등록하면 이 비용이 사라진다고 알려줍니다. 이름은 붙일 수 없으니 주소만 정확히 넣으면 됩니다 — 자비스가 주소로 알아서 연결합니다.
...
이 선언이 아예 없으면(즉 반품지 자체가 하나도 없으면) 자비스도 물리적으로 등록할 주소가 없어 막힙니다 — 이건 토스 정책이 아니라 전자상거래법이 요구하는 최소 요건이라 자동화로 풀 수 없는 유일한 지점입니다.

#### 반품 비용은 예측하지 않고 상한으로 잡습니다

셀러 경유(반품 안내가 없는 상품, 또는 공급처 주소 미등록으로 강제 폴백된 상품)일 때는 왕복 택배비가 실제로 발생합니다. 자비스는 이걸 예측하지 않고 **반품률 상한(8%)** 을 곱해 판매 1건당 충당금으로 마진에서 미리 뺍니다. 빼고도 남을 때만 팝니다. 정산 데이터가 쌓이면 실측 반품률이 이 상한을 대체합니다.
...
인센티브가 준수율 **100%**를 요구하므로 정상출고율 기준을 `SAME_DAY_MIN_FULFILLMENT_RATE_PCT = 98`로 둔다. 종전 80%는 "5건 중 1건 지연"을 허용해 오늘출발 전략에서 재앙이다. `meetsSupplierPolicy()`는 이 기준으로 "오늘출발을 약속해도 되는가·인센티브를 가정해도 되는가"만 판정하고, **출고율 미확인은 여기서만 탈락**시킨다 — 배송 약속은 추측할 수 없기 때문이다.

**소싱 여부(등록해도 되는가)는 이제 다른 함수가 담당한다 — `isSupplierViableForSourcing()`.** 도매꾹/도매매 API는 등급·출고속도 필드를 안 주는 공급처가 대부분이다. 예전엔 `meetsSupplierPolicy()`를 소싱 하드 게이트로도 같이 썼는데, 그러면 마진·반품정책이 멀쩡한 상품도 "등급 미확인" 하나로 통째로 등록이 막혔다. 지금은 미확인은 통과시키고, **실측으로 확인된 위험**(출고율 70% 미만, 배송 지연 확정)만 막는다. 결과적으로:

- 등급 미확인 상품 → 소싱됨, 다만 "오늘출발" 약속·배송 인센티브 가정은 안 함(수수료 8%로 보수 계산)
- 1등급·당일발송 확인 상품 → 소싱됨 + 오늘출발 약속 + 인센티브 가정 가능
- 출고율 70% 미만으로 확인된 공급처 → 소싱 자체를 막음 (실측된 위험)
