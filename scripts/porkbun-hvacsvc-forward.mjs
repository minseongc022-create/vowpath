/**
 * hvacsvc.link → effiroad.com: Porkbun root ALIAS + URL forwards.
 * De-index legacy marketing domain (Google still shows /get-started).
 *
 *   export PORKBUN_API_KEY=...
 *   export PORKBUN_SECRET_KEY=...
 *   node scripts/porkbun-hvacsvc-forward.mjs
 */
const DOMAIN = "hvacsvc.link";
const FORWARD_TARGET = "https://effiroad.com";
const ROOT_ALIAS = "uixie.porkbun.com";
const GSC_TXT = process.env.GOOGLE_SITE_VERIFICATION_HVACSVC?.trim();
...
async function ensureGoogleSiteVerificationTxt() {
  if (!GSC_TXT) {
    console.log(
      "○ Skip GSC TXT — set GOOGLE_SITE_VERIFICATION_HVACSVC (Search Console → DNS verify for hvacsvc.link)",
    );
    return true;
  }
...
async function main() {
  ...
  console.log("\n=== Porkbun hvacsvc.link setup (legacy → effiroad.com) ===\n");
  ...
  console.log(`\n${ok}/4 steps OK`);
  console.log(
    "\nAfter DNS propagates: npm run vercel:domains (link hvacsvc.link) → redeploy → npm run seo:ping",
  );
  console.log(
    "Google Search Console: Removals → Temporary removal → prefix https://hvacsvc.link/\n",
  );
