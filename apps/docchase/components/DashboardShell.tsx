...
export function DashboardShell({
  ...
  return (
    <div className="min-h-screen bg-mesh-hero">
      <header className="border-b border-paper-line/80 bg-paper-card/90 backdrop-blur">
        <div className="sc-container flex h-14 items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <span className="hidden rounded-full border border-paper-line bg-white px-2.5 py-0.5 text-xs text-ink-muted sm:inline">
              {officeName || "데모 사무소"}
            </span>
          </div>
          <button
            type="button"
            className="sc-btn-ghost text-sm"
            onClick={() => {
      ...
      <main className="sc-container py-8">{children}</main>
    </div>
  );
}
