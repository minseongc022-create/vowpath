  ...
  function exportCsv() {
    const rows = [
      ...
      ...state!.orders.map((o) => {
        ...
        const amount = o.items.reduce((s, i) => s + i.qty * i.unitPrice, 0);
        return `"${p?.name || ""}",${o.number},"${o.title}",${o.status},${amount},"${p?.clientName || ""}"`;
      }),
    ];
    ...
    a.href = url;
    a.download = "추가확정_변경대장.csv";
    a.click();
    URL.revokeObjectURL(url);
    commit(withActivity(state!, "변경대장 CSV 다운로드"), "CSV 저장");
  }
  ...
  return (
    <DashboardShell companyName={state.profile.companyName}>
      ...
      <div className="mt-6 max-w-lg sc-card p-5">
        <h2 className="text-sm font-semibold text-ink">변경대장 CSV</h2>
