...
export default function SignupPage() {
  ...
  const [companyName, setCompanyName] = useState("동방전기");
  const [ownerName, setOwnerName] = useState("최민재");
  const [email, setEmail] = useState("demo@giseong.kr");
  ...
  function start(e: React.FormEvent) {
    ...
    saveState(state);
    localStorage.setItem("giseong.session", JSON.stringify({ email: state.profile.email, at: Date.now() }));
    ...
    <div className="min-h-screen bg-mesh-hero">
      ...
      <main className="sc-container py-10 sm:py-14">
        <div className="mx-auto max-w-3xl">
          ...
          <p className="mt-3 text-sm text-ink-muted">플랜을 고르고 미승인 기성부터 확인하세요.</p>
